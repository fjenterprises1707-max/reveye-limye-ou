...
const SERVICES = [
  {
    ...
    desc: { kr: "Ou pale avèm, m' koute ou epi nou jwenn ansanm sa ki pi bon pou ou. Chak sesyon adapte selon bezwen ou.", fr: "Vous me parlez, j'écoute et nous trouvons ensemble ce qui est le mieux pour vous.", en: "You speak, I listen and together we find what's best for you.", es: "Me hablas, te escucho y juntos encontramos lo mejor para ti." },
  ...
  },
  {
    ...
    desc: { kr: "Pwodui yo enkli. M' ap akonpaye ou nan tout pwosesis la pou yon netwayaj konplè ak gerizon pwofon.", fr: "Produits inclus. Je vous accompagne tout au long du processus.", en: "Products included. I guide you through the entire process for complete cleansing.", es: "Productos incluidos. Te acompaño durante todo el proceso." },
  ...
  },
  {
    ...
    desc: { kr: "Beny adapte selon sa ou bezwen — netwayaj, pwoteksyon, atire abondans, oswa gerizon kè.", fr: "Bain adapté selon votre besoin — purification, protection, abondance ou guérison.", en: "Bath tailored to your need — cleansing, protection, abundance or healing.", es: "Baño adaptado a tu necesidad — purificación, protección, abundancia o sanación." },
...
const BRANDS = [
  ...
  },
  {
    ...
    color: "#5C7A3E",
    gradient: "linear-gradient(135deg, #1a2a0a, #2d4a1a)",
    icon: "🌿",
    packages: [],
...
const SHOP_ITEMS = [
  { id: 1, icon: "🏠", name: { kr: "Standard Package", fr: "Standard Package", en: "Standard Package", es: "Standard Package" }, brand: "Ancestral Light", official: "$77.77", referral: "$55.55", desc: { kr: "Kit netwayaj kay espirityèl", fr: "Kit nettoyage maison spirituel", en: "Spiritual house cleaning kit", es: "Kit limpieza espiritual del hogar" }, badge: "Ancestral" },
  { id: 2, icon: "✨", name: { kr: "Premium Package", fr: "Premium Package", en: "Premium Package", es: "Premium Package" }, brand: "Ancestral Light", official: "$144.44", referral: "$105.55", desc: { kr: "Kay + Beny espirityèl", fr: "Maison + Bain spirituel", en: "House + Spiritual bath", es: "Casa + Baño espiritual" }, badge: "Ancestral" },
  { id: 3, icon: "👑", name: { kr: "Extra Premium Package", fr: "Extra Premium Package", en: "Extra Premium Package", es: "Extra Premium Package" }, brand: "Ancestral Light", official: "$333.33", referral: "$250.77", desc: { kr: "Kay + Beny + Meditasyon + Gidans", fr: "Maison + Bain + Méditation + Guidance", en: "House + Bath + Meditation + Guidance", es: "Casa + Baño + Meditación + Guía" }, badge: "Ancestral" },
  { id: 4, icon: "🌿", name: { kr: "Granny Tea", fr: "Granny Tea", en: "Granny Tea", es: "Granny Tea" }, brand: "Granny Tea", official: "—", referral: "—", desc: { kr: "Pwochen lanse — Zèb natirèl ayisyen", fr: "Prochainement — Herbes naturelles haïtiennes", en: "Coming soon — Natural Haitian herbs", es: "Próximamente — Hierbas naturales haitianas" }, badge: "Granny Tea", soon: true },
];
...
export default function App() {
  ...
  const HomePage = () => (
    <div>
      ...
      <div style={{textAlign:"center",padding:"30px 0 20px"}}>
        ...
        <p style={{color:C.muted,fontSize:13}}>{t.home.sub}</p>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        {[
          ...
          <div key={i} onClick={()=>setPage(item.page)}
            style={{...s.card,textAlign:"center",cursor:"pointer",padding:16,marginBottom:0,border:`1px solid ${item.col}33`,background:`linear-gradient(135deg, ${C.card}, ${item.col}08)`}}>
      ...
      {BRANDS.map(br=>(
        <div key={br.id} onClick={()=>{setPage("brands");setActiveBrand(br.id);}}
          style={{...s.card,cursor:"pointer",border:`1px solid ${br.color}44`,background:`linear-gradient(135deg, ${C.card}, ${br.color}08)`,display:"flex",gap:14,alignItems:"center",marginBottom:10}}>
    ...
    return (
      <div>
        ...
        {SERVICES.map(svc=>{
          ...
          return (
            <div key={svc.id} onClick={()=>setSelSvc(sel?null:svc)}
              style={{...s.goldCard,cursor:"pointer",border:sel?`2px solid ${svc.color}`:`1px solid ${svc.color}33`,transition:"all .2s"}}>
              <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
                <div style={{width:50,height:50,borderRadius:14,background:svc.color+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{svc.icon}</div>
                <div style={{flex:1}}>
              ...
              {sel && (
                <div onClick={e=>e.stopPropagation()} style={{marginTop:16,borderTop:`1px solid ${C.border}`,paddingTop:16}}>
                  {/* Date */}
                  <div style={{fontWeight:700,fontSize:12,color:C.text,marginBottom:8,letterSpacing:0.5}}>📅 {t.services.selectDate}</div>
                  <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8,marginBottom:14}}>
                    {dates.map(d=>(
                      <button key={d} onClick={()=>setSelDate(d)}
                        style={{padding:"6px 11px",borderRadius:18,border:`1.5px solid ${selDate===d?svc.color:C.border}`,background:selDate===d?svc.color:"transparent",color:selDate===d?C.white:C.text,cursor:"pointer",fontSize:10,whiteSpace:"nowrap",flexShrink:0,fontFamily:"inherit"}}>
                        {d}
                      </button>
                    ))}
                  </div>
                  {/* Time */}
                  <div style={{fontWeight:700,fontSize:12,color:C.text,marginBottom:8,letterSpacing:0.5}}>🕐 {t.services.selectTime}</div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:16}}>
                    {TIMES.map(time=>(
                      <button key={time} onClick={()=>setSelTime(time)}
                        style={{padding:"6px 12px",borderRadius:18,border:`1.5px solid ${selTime===time?svc.color:C.border}`,background:selTime===time?svc.color:"transparent",color:selTime===time?C.white:C.text,cursor:"pointer",fontSize:11,fontFamily:"inherit"}}>
                        {time}
                      </button>
                    ))}
                  </div>
                  {selDate && selTime && (
                    <>
                      <div style={{fontWeight:700,fontSize:12,color:C.text,marginBottom:8,letterSpacing:0.5}}>💳 Peman</div>
        ...
        <div key={br.id} style={{...s.goldCard,border:`1px solid ${br.color}44`,marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:14,alignItems:"center",marginBottom:14}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:C.muted,marginTop:2,letterSpacing:0.5}}>{br.tagline[lang]}</div>
          ...
          {br.packages.length > 0 ? (
            <>
              {br.packages.map(pkg=>(
                <div key={pkg.num} style={{background:`linear-gradient(135deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01))`,border:`1px solid ${pkg.color}33`,borderRadius:12,padding:"14px 16px",marginBottom:10}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:C.muted,marginTop:4,marginLeft:32}}>{pkg.sub}</div>
                    </div>
                  </div>
          ...
          <div key={key} style={{...s.goldCard,border:isJoined?`2px solid ${ch.color}`:`1px solid ${ch.color}33`}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setJoined(j=>({...j,[key]:!j[key]}))}
                style={{...s.btn(ch.gradient),flex:1,padding:"12px 0"}}>
                {isJoined?t.challenges.cont:t.challenges.join}
              </button>
              <button onClick={()=>setExpandedChallenge(isExp?null:key)}
                style={{...s.btnOutline(ch.color),padding:"12px 14px"}}>{isExp?"▲":"▼"}</button>
            </div>
    ...
    return (
      <div>
        ...
        <div style={{display:"flex",gap:10,marginBottom:18}}>
          {["c7","c21"].map(key=>(
            <button key={key} onClick={()=>setPlaylistTab(key)}
              style={{flex:1,padding:"10px 0",borderRadius:10,border:playlistTab===key?"none":`1.5px solid ${CHALLENGES[key].color}`,background:playlistTab===key?CHALLENGES[key].gradient:"transparent",color:playlistTab===key?C.white:CHALLENGES[key].color,fontWeight:700,fontSize:12,cursor:"pointer",transition:"all .2s",fontFamily:"inherit"}}>
              {CHALLENGES[key].icon} {key==="c7"?t.music.c7:t.music.c21}
            </button>
          ))}
        </div>
        <div style={{...s.goldCard,borderLeft:`3px solid ${ch.color}`,marginBottom:16}}>
          <div style={{fontWeight:700,fontSize:14,color:C.gold}}>{key==="c7"?t.music.c7:t.music.c21}</div>
          <div style={{fontSize:11,color:C.muted,marginTop:2}}>{ch.playlist.length} tracks • {ch.title[lang]}</div>
        </div>
        {ch.playlist.map((track,i)=>{
          const tid=playlistTab+i;
          const isP=playingId===tid;
          const col=trackColors[i];
          return (
            <div key={i} style={{...s.card,display:"flex",alignItems:"center",gap:12,padding:14,marginBottom:8}}>
              <div style={{width:40,height:40,borderRadius:10,background:col+"22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0,fontWeight:700,color:col}}>{i+1}</div>
              <div style={{flex:1}}>
              ...
              <button onClick={()=>setPlayingId(isP?null:tid)} style={{width:36,height:36,borderRadius:"50%",background:isP?col:col+"22",border:"none",cursor:"pointer",fontSize:14,color:isP?C.white:col,flexShrink:0,transition:"all .2s"}}>
                {isP?"⏸":"▶"}
              </button>
            </div>
          );
        ...
        {SHOP_ITEMS.map(item=>{
          ...
          return (
            <div key={item.id} style={{...s.goldCard,padding:14,display:"flex",flexDirection:"column",position:"relative",marginBottom:0}}>
              <span style={{...s.badge(item.badge==="Ancestral Light"?C.primary:C.success),position:"absolute",top:10,right:10,fontSize:8}}>{item.badge}</span>
  ...
  return (
    <div style={s.app}>
      ...
      {/* Content */}
      <div style={s.page}>{currentPage}</div>
